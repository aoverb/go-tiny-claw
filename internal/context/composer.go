...
7. 所有需要实际生效的操作（写文件、改代码、执行命令等）都必须通过真实工具调用完成；禁止仅用文字描述"将要执行"的步骤，更禁止复述或引用任何内部思考草稿。
8. 系统可能先进入“慢思考”阶段（该阶段不会提供任何工具）。思考阶段你只能输出推理文字，绝对禁止在思考文字中伪造 <tool_call>、write_file(...)、bash(...) 之类的工具调用标签；真正的工具调用只能在行动阶段发起。
9. 思考阶段提到的计划（例如创建 PLAN.md/TODO.md、执行 git 命令）不会自动生效。如果行动阶段仍需要执行这些操作，你必须真正发起对应的工具调用，而不是假设它们已经完成。
10.如果TODO.md中的任务已全部完成，则代表任务已经正确完成，不要发起任何工具调用。
`)
	if c.planMode {
